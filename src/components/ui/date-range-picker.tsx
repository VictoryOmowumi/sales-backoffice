...
export const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const getPresetRange = React.useCallback((presetName: string): DateRange => {
    ...
    switch (presetName) {
      ...
      case "thisWeek": {
        ...
        return { from: thisWeekStart, to: endToday };
      }
      case "lastWeek": {
  ...
  const setPreset = (preset: string): void => {
    ...
    if (newRange.from) {
      const fromMonth = new Date(newRange.from);
      const toMonth = new Date(fromMonth);
      toMonth.setMonth(fromMonth.getMonth() + 1);
      setCalendarMonths([fromMonth, toMonth]);
    }
  };
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={cn(
            "w-full sm:w-[300px] justify-between text-left hover:text-muted-foreground font-normal text-wrap",
            className,
          )}
        >
              ...
              {/* Two calendars side by side for desktop */}
               <Calendar
                 ...
                 onMonthChange={(month) => {
                   ...
                   setCalendarMonths([newMonth, nextMonth]);
                 }}
               ...
               <Calendar
                 ...
                 onMonthChange={(month) => {
                   ...
                   prevMonth.setMonth(newMonth.getMonth() - 1);
                   setCalendarMonths([prevMonth, newMonth]);
                 }}
                 className="border rounded-md"
               />
            </div>
